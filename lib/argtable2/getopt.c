...
static void store_args(int argc, char *const *argv)
{
	/* XXX This is no good solution.  We should rather copy the args so
	   that we can compare them later.  But we must not use argtable2_malloc(3).  */
	original_argc = argc;
	original_argv = argv;
}
...
#ifdef _LIBC
#define NONOPTION_P (argv[optind][0] != '-' || argv[optind][1] == '\0' || (optind < nonoption_flags_len && nonoption_flags[optind] == '1'))
#else
