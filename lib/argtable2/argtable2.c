...
#ifndef NDEBUG
static void dump_longoptions(struct longoptions *longoptions)
{
    int i;
...
static struct longoptions *alloc_longoptions(struct arg_hdr **table)
{
    ...
    result = (struct longoptions *)argtable2_malloc(nbytes);
    if (result)
    {
        int option_index = 0;
        char *store;
...
static void arg_parse_tagged(int argc, char **argv, struct arg_hdr **table, struct arg_end *endtable)
{
    ...
    if (!longoptions || !shortoptions)
    {
        ...
        argtable2_free(shortoptions);
        argtable2_free(longoptions);
        return;
    }
...
int arg_parse(int argc, char **argv, void **argtable)
{
    ...
    argvcopy = argtable2_malloc(sizeof(char *) * argc);
    if (argvcopy)
    {
        int i;
 ...
 * Author: Uli Fouquet
 */
static void arg_print_formatted(FILE *fp, const unsigned lmargin, const unsigned rmargin, const char *text)
{
...
/*
 * arg_argtable2_free() is deprecated in favour of arg_freetable() due to a flaw in its design.
 ...
 * deallocated by arg_argtable2_free().
 * Despite the unlikeliness of the problem occurring, and the even unlikelier event
 * that it has any deliterious effect, it is fixed regardless by replacing arg_argtable2_free()
 * with the newer arg_freetable() function.
 * We still keep arg_argtable2_free() for backwards compatibility.
 */
void arg_argtable2_free(void **argtable)
{
    struct arg_hdr **table = (struct arg_hdr **)argtable;
    int tabindex = 0;
    int flag;
    /*printf("arg_argtable2_free(%p)\n",argtable);*/
    do
    {
        /*
